...
export const FigmaCard: React.FC<Props> = memo(({ title, url, date, favicon, thumbnail, site, open }) => {
  const handleClick = useCallback(() => {
    if (isBrowser) {
      const pathname = window.location.pathname;
      const hash = window.location.hash;
      history.pushState({}, '', `${pathname}${hash}?open=${date}`);
      open();
    }
  }, [url]);
  return (
    <div
      role="button"
      className="flex items-center w-full border dark:border-zinc-500 hover:bg-zinc-50 hover:dark:bg-zinc-800 rounded overflow-hidden h-[120px] hover:cursor-pointer"
      onClick={handleClick}
    >
